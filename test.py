...
def plot_lengths(array_of_ints):
    ...
    for num in array_of_ints:
        ...
        count = 0 if count == len(behavior_ref) - 1 else count + 1

    return path_str


def plot_lengths_curved(array_of_ints):
    # Turn left 90 degrees each time
    # behavior_ref = ['q h -', 'v ', 'h ', 'v -']
    path_str = 'M50 20j'
    count = 0
    for num in array_of_ints:
        if count == 1:
            move = 'q%s %s, %s %s' % (num * 2, num * 2, num * 5, num * 5)
        else:
            move = 'td%s d%s' % (num, num)
        path_str = ' '.join([path_str, move])
        # count = 0 if count == len(behavior_ref) - 1 else count + 1
    print path_str

    # path1 = QuadraticBezier(num, num)
    return path_str


def build_path_str(filepath):
    lens = get_sentence_lengths(filepath)
    doubled = [x * 2 for x in lens]
    path_str = plot_lengths_curved(doubled)
    print parse_path(path_str)
